...
def fill_strato_tropo(strato_co, tropos_co):
    '''
    the array has nans when monthly meaned, this fills the nans and replaces 0's with nearest level value
    '''

    strato_co = np.nan_to_num(strato_co)
    tropos_co = np.nan_to_num(tropos_co)
    ...
    for i in xrange(ni):
        for j in xrange(nj):
            for k in xrange(nk):
                if tropos_co[k,i,j] == 0:
                    tropos_co[k:,i,j] = tropos_co[k-1, i, j]
                    break

    for i in xrange(ni):
        for j in xrange(nj):
            for k in xrange(nk - 1, -1, -1):  # goes through levels backwards
                if strato_co[k,i,j] == 0:
                    strato_co[:k+1,i,j] = strato_co[k+1, i, j]
                    break

    print "That took {0} seconds.".format(str(time.time() - start_time))
    return tropos_co, strato_co


def get_from_binned(m_int, y_int, ts, species_data):
    '''
    gets the binned coordinates given the year int and month int, 
    then converts them to co datapoints based on binned indices
    '''
    global strato_floor, tropo_ceiling
    npy = '/space/hall1/sitestore/eccc/aq/r1/alh002/FST_STORAGE/tropo_strato_files/BINNING_1x/{0}-tropo_{1}-{2}.npy'.format(ts, y_int, m_int)
    binned = np.load(npy)
    binned = binned[:59]
    
    species_data = species_data[:,::-1]
    print "The binned shape is {0}".format(binned.shape)
    tropos_co = np.zeros(binned.shape)
    strato_co = np.zeros(binned.shape)

    for i in xrange(ni):
        for j in xrange(nj):
            for k in xrange(nk):
                if binned[k,i,j] == 1:
                    tropos_co[k,i,j] = species_data[ts,k,j,i]
                else:
                    strato_co[k,i,j] = species_data[ts,k,j,i]
...
for m_int in xrange(7,8):
    if m_int >= 4:
        continue
    all_year_tropos_co = np.zeros((len(years), nk, ni, nj))
    all_year_strato_co = np.zeros((len(years), nk, ni, nj))
    ...
    for y_int, year in enumerate(years):
        ...
        species_data = splice_month(pyg.open('/space/hall1/sitestore/eccc/aq/r1/alh002/NCDF/SPECIES/CO/{0}.nc'.format(year)).co, m_int)

        species_data = species_data[:,:,:nj]
        species_data = species_data[:,:,::-1]
    
        tropos_co = np.zeros((len(species_data), nk, ni, nj))
        strato_co = np.zeros(tropos_co.shape)
        print "Set up tropo/strato arrays with size {0}".format(strato_co.shape)

        # fills the tropo_co ts
        print "Filling up co"
        for t in xrange(len(species_data)):
            tropos_co[t], strato_co[t] = get_from_binned(m_int, y_int, t, species_data)

        print "Acquiring monthly mean for year {0}".format(year)
        tropos_co = monthly_mean_data(tropos_co)
        strato_co = monthly_mean_data(strato_co)
    
        all_year_tropos_co[y_int] = tropos_co
        all_year_strato_co[y_int] = strato_co
    
    print "Acquiring monthly means over {0} years.".format(len(years))
    all_year_tropos_co = np.mean(all_year_tropos_co, axis=0)
    all_year_strato_co = np.mean(all_year_strato_co, axis=0)
    all_year_tropos_co, all_year_strato_co = fill_strato_tropo(all_year_strato_co, all_year_tropos_co)

    print "Building FST file"
    build_fst(params0, all_year_tropos_co, all_year_strato_co, m_int)
